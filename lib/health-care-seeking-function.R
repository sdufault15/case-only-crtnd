# differential health care seeking behavior function


dhcsb <- function(dF, rrIN, period, n1 = 1000, ratio = 4, hcsb = 0.1){
  # 'dF' is a matrix/dataframe containing cluster, case and control data, corresponding time periods and treatment allocations
  ...
  # 'hcsb' is the health care-seeking behavior effect
  iter1 <- 1
  
  for (j in period){
    df1 <- dF %>% filter(Period == period) # subsetting to the specific period
    m <- nrow(df1)/2
    txDtaF <- txtSet(df1)
